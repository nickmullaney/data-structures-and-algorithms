...
const getHouses = (arr) => {
  ...
  // houses.push(arr.map(char => char.house));
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  // return Object.values(character).includes('children');
  let children = 0;
  arr.forEach(person => {
    if (person.name === character) {
      Object.keys(person).forEach((keys, idx) => {
        if (keys === 'children') {
  ...
  //     return(Object.values(val).filter(current=> typeof current === 'object')).length > 0;
  //   }}
};
