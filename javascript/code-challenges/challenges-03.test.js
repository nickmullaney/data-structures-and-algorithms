...
const typeNum = (arr) => {
  return arr.filter(numOnly => typeof numOnly === 'number');
};
...
const containsAnd = (arr) => {
  return arr.filter(stringOnly => stringOnly.includes('and'));
};
...
const oddValues = (arr) => {
  return arr.filter(oddOnly => oddOnly % 2 !== 0);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(notAlike => !forbiddenValues.includes(notAlike));
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // This iterates and filters greater than using if greaterthan.basestat is greater than input
  return arr.filter(greaterThan => greaterThan.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) => {
  //fixed
  // return arr.filter(nameOnly => nameOnly.baseStat > minBaseStat).map(nameOnly => nameOnly.stat.name);
  // or
  return getBaseStatGreaterThan(arr,minBaseStat).map(nameOnly => nameOnly.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(noKids => !('children' in noKids));
};
